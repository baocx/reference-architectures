configuration SQLServerPrepareDsc
{
    ...
    Import-DscResource -ModuleName StorageDsc, xSmbShare, xComputerManagement, xNetworking, xActiveDirectory, xFailoverCluster, SqlServer, SqlServerDsc
    ...
    Node localhost
    {
        ...
        SqlLogin AddDomainAdminAccountToSqlServer
        {
        ...
        xADUser CreateSqlServerServiceAccount
        {
            ...
            Ensure                        = "Present"
            DependsOn                     = "[SqlLogin]AddDomainAdminAccountToSqlServer"
        }

        SqlLogin AddClusterSvcAccountToSqlServer
        {
		...
		SqlRole AddDomainAdminAccountToSysAdmin
        {
			Ensure           = "Present"
			...
			ServerName       = "$env:COMPUTERNAME"
			InstanceName     = "MSSQLSERVER"
			DependsOn        = "[SqlLogin]AddDomainAdminAccountToSqlServer","[SqlLogin]AddClusterSvcAccountToSqlServer"
        }
        ...
        SqlServiceAccount SetServiceAcccount_User
        {
            ...
            RestartService  = $true
            DependsOn       = "[SqlRole]AddDomainAdminAccountToSysAdmin"
        }
        ...
        if ($ClusterOwnerNode -eq $env:COMPUTERNAME) { 
            ...
            SqlAG CreateAG
            {
                ...
                DependsOn            = "[SqlEndpoint]HADREndpoint","[SqlRole]AddDomainAdminAccountToSysAdmin"
            }
            ...
            SqlAGDatabase AddDatabaseToAG
            {
                ...
                PsDscRunAsCredential    = $DomainCreds
                DependsOn               = "[SqlAG]CreateAG", "[SqlDatabase]Create_Database"
            }
